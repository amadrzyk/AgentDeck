...
| **Ulanzi D200H** | HW (HID deck) | SigmaStar SSD210 | nv3052c LCD · 960×540 (14 keys) | USB HID | USB | ✅ |
| **Elgato Stream Deck+** | HW (HID deck) | Elgato (내장) | LCD keys + dials + touch strip | WebSocket :9120 | USB(호스트) → 네트워크 | ✅³ |
...
¹ **TC001 ⚠️ 는 샌드박스 제약이 아니라 구현 갭**: TC001 은 다른 ESP32 보드와 동일하게 USB serial / WiFi WS 로 붙어 state-JSON 을 자기 렌더하며 `com.apple.security.device.serial` entitlement 로 이미 커버된다(`tui-dashboard.test.ts` 가 TC001 을 serial board 로 보고). 현재 Swift(App Store) 데몬에 `led8x32` 경로가 미검증이라 CLI-tier. **"ADB" 표기는 전부 legacy/dead code** — `AdbDeviceClass.ulanziTc001` 와이어 클래스를 emit 하는 producer 가 없어 `TopologyRail.pixelDisplaySection` 의 ADB 분기는 죽어 있고, 라이브 경로는 serial 이다. appstore-feature-matrix·`AdbModule.swift`·`AgentState.swift`·`TopologyRail.swift` 의 ADB 표기는 본 정리에서 정정됨(dead-code 제거는 후속 권장). 하드웨어 검증 후 ✅ 승격 가능.
...
³ Elgato Stream Deck 앱 별도 설치 필요 (플러그인 호스트).
...
| 보드 | env | 모델 (제조사) | SoC | 해상도 | 디스플레이 IC | 터치 IC | Flash | PSRAM | USB-UART |
|---|---|---|---|---|---|---|:---:|:---:|---|
| **IPS 3.5"** | `ips35` | JC3248W535 (Guition) | ESP32-S3 | 480×320 | AXS15231B (QSPI) | 통합(AXS15231B) | 16MB | ✅ (qio_opi) | Native USB JTAG |
| **Round AMOLED 1.8"** | `amoled` | JC3636W518 (Guition) | ESP32-S3 | 360×360 (원형) | ST77916 (QSPI) | CST816S (I2C) | 8MB | ✅ | Native USB JTAG |
| **86 Box 4"** | `box_86` / `rgb48` | ESP32-S3-4848S040 (Guition) | ESP32-S3 | 480×480 | ST7701 (RGB 16-bit 병렬) | GT911 (I2C) | 16MB | ✅ | CH340 |
| **TTGO T-Display 1.14"** | `ttgo` | LilyGO T-Display | ESP32-D0WDQ6 (classic) | 135×240 | ST7789 (SPI) | 없음 (버튼 2개) | 16MB | ❌ | CH340⁴ |
| **ESP32-C6 1.47"** | `esp32_c6_147` | Waveshare ESP32-C6-LCD-1.47" | ESP32-C6 (RISC-V, single-core) | 172×320 | ST7789 (SPI) | 없음 (BOOT 버튼) | 4MB | ❌ | Native USB CDC (HWCDC) |
| **IPS 10.1"** | `ips10` | JC8012P4A1C (Guition) | ESP32-P4NRW32 (RISC-V dual-core 400MHz) | 800×1280 (세로) | JD9365 (MIPI-DSI) | GSL3680 (I2C) | 4MB⁵ | ✅ 32MB | CH340 |
...
상용 USB HID 매크로 데크. ADB/펌웨어 변조 없이 stock HID 프로토콜로 구동. 출처: [docs/plugin-conventions.md](plugin-conventions.md) §D200H, memory `d200h-device.md`.
...
| **드라이버** | Swift `D200hHidModule.swift`(IOKit IOHIDManager) / Node `bridge/src/d200h/`(node-hid). daemon-only |
| **App Store** | ✅ — 주경로 Ulanzi Studio 플러그인(`plugin-ulanzi`), 폴백 direct-HID(`com.apple.security.device.usb`) |
...
| 항목 | 사양 |
|---|---|
| **하드웨어** | LCD 키 8개(2×4) + 로터리 인코더 4개 + LCD 터치 스트립 |
| **연결** | 호스트 USB → Elgato Stream Deck 앱 플러그인 → daemon WebSocket(:9120) |
| **플러그인** | Stream Deck SDK v2, UUID `bound.serendipity.agentdeck`. 프로파일 `agentdeck-sdplus`(DeviceType 7) / `agentdeck-sd`(DeviceType 0, 표준 SD) |
| **인코더 매핑** | E1 유틸리티(볼륨/마이크/미디어/타이머) · E2 액션(프롬프트/옵션) · E3 터미널 · E4 음성. 음성 takeover 시 E2–E4 wide-canvas 병합 |
| **이벤트** | 양방향, BridgeEvent 13종 전부 |
...
출처: [docs/devices.md](devices.md), `bridge/src/index.ts` (`broadcastSse`). (참고: OpenClaw **Gateway**(WS custom :18789, Ed25519)는 dashboard surface 가 아니라 **upstream agent adapter** 라 본 표에서 제외.)
...
요약: **진짜 sandbox 제약은 e-ink(adb) 경로뿐**이고 CLI 대체 경로가 정당하다. **TC001 과 Android 태블릿의 ❌ 는 구현/분류 갭**이라 본 문서에서 ⚠️ 로 정정했으며, upstream 문서(appstore-feature-matrix.md)와 `AdbModule.swift`·`AgentState.swift`·`TopologyRail.swift` 주석의 ADB 오기도 본 정리에서 함께 정정했다(legacy dead-code 제거는 후속 권장).
