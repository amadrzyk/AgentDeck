...
export function renderUsageOverview(data: UsageModeData): string {
  ...
  return svgWrap(`
    ...
    <text x="100" y="16" text-anchor="middle" font-family="Arial,sans-serif" font-size="12" fill="#94a3b8">USAGE</text>
    <text x="18" y="38" font-family="Arial,sans-serif" font-size="11" fill="#94a3b8">5H</text>
    <text x="42" y="38" font-family="monospace" font-size="12" fill="${c5}">${esc(gaugeBar(pct5, 8))}</text>
    <text x="155" y="38" font-family="Arial,sans-serif" font-size="12" font-weight="bold" fill="#ffffff">${Math.round(pct5)}%</text>
    <text x="190" y="38" text-anchor="end" font-family="Arial,sans-serif" font-size="10" fill="#64748b">${esc(reset5)}</text>
    <text x="18" y="62" font-family="Arial,sans-serif" font-size="11" fill="#94a3b8">7D</text>
    <text x="42" y="62" font-family="monospace" font-size="12" fill="${c7}">${esc(gaugeBar(pct7, 8))}</text>
    <text x="155" y="62" font-family="Arial,sans-serif" font-size="12" font-weight="bold" fill="#ffffff">${Math.round(pct7)}%</text>
    <text x="190" y="62" text-anchor="end" font-family="Arial,sans-serif" font-size="10" fill="#64748b">${esc(reset7)}</text>
    <rect x="0" y="96" width="${W}" height="4" rx="2" fill="${gaugeColor(Math.max(pct5, pct7))}" opacity="0.6"/>
  `);
}

/** Detail view for a single rate limit (5h or 7d) */
export function renderUsageDetail(data: UsageModeData, page: '5h' | '7d'): string {
  ...
  return svgWrap(`
    ...
    <text x="100" y="48" text-anchor="middle" font-family="monospace" font-size="14" fill="${color}">${esc(gaugeBar(pct, 12))}</text>
    <text x="100" y="74" text-anchor="middle" font-family="Arial,sans-serif" font-size="20" font-weight="bold" fill="#ffffff">${Math.round(pct)}%${reset ? ` \u00B7 ${esc(reset)}` : ''}</text>
...
export function renderUsageExtra(data: UsageModeData): string {
  ...
  return svgWrap(`
    ...
    <text x="100" y="20" text-anchor="middle" font-family="Arial,sans-serif" font-size="14" fill="#94a3b8">EXTRA USAGE</text>
    <text x="100" y="48" text-anchor="middle" font-family="monospace" font-size="14" fill="${color}">${esc(gaugeBar(pct, 12))}</text>
    <text x="100" y="74" text-anchor="middle" font-family="Arial,sans-serif" font-size="20" font-weight="bold" fill="#ffffff">${Math.round(pct)}%</text>
    <rect x="0" y="96" width="${W}" height="4" rx="2" fill="${color}" opacity="0.6"/>
  `);
}
